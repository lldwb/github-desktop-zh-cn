...
**切换版本**（工具栏「更新管控」右边）在**本机已安装的 GitHub Desktop** 之间切换——官方升级后旧的 `app-<版本>` 目录会留着，同一台机器上可能并存多个。点开就是一份版本列表，**默认只列有汉化字典的版本**，勾上「显示没汉化的版本」才连没字典的一起列；每项带版本号、状态标签（当前 / 无字典 / 自定义目录）与所在目录，点一项即切换。选中的版本就是此后汉化 / 还原 / 更新管控 / 字典表格的对象；切换时会**默认勾选「同时禁止该版本自动更新」**，把选定的版本钉住、免得被官方更新悄悄换走（不想上锁就取消勾选）。
