...
- **工具链**（`scripts/`，Node.js 零依赖，仅内置模块）：
  ...
  - `update.js`：工具自更新——查 latest release → 按 `<平台>-<架构>` 后缀选资产 → 下载并校验文件头（MZ / Mach-O / ELF）→ 改名替换自身 → 重启新版本；启动时 `cleanup()` 清理上次的 `.old` 残留。CLI 侧是替换自身；**GUI 侧是下载安装包交给用户装**（`pickGuiAsset()` 只认 `-gui-` 名字，Windows `-setup.exe` 直接 spawn、macOS `.dmg` 与 Linux `.deb` 交 `open` / `xdg-open`），两者命名与行为都不同、别混。
