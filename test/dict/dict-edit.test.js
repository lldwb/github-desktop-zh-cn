// 字典写入口（scripts/dict/dict-edit.js）单测：只读 API、校验清单、事务写入与失败回滚
//
...
const dictEdit = require('../../scripts/dict/dict-edit');
const scratch = require('../fixtures/scratch');

// 夹具版本号只在此处引用，定义在 test/fixtures/scratch.js（见该目录 README）
const VERSION = scratch.FIXTURE_VERSION;
...
// 每个用例自带清理：断言失败也不会把临时字典留在仓库里（清理方式统一定义在 test/fixtures/scratch.js）
const isolate = scratch.isolate;
