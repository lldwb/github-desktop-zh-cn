...
function get(path) {
  return new Promise((resolve, reject) => {
    https.get(
      { host: 'api.github.com', path, headers: { 'User-Agent': 'ci-status', Accept: 'application/vnd.github+json' } },
      (res) => {
        let body = '';
...
(async () => {
  const runs = await get(`${R}/actions/workflows/build.yml/runs?per_page=2`);
  for (const r of runs.json.workflow_runs || []) {
    ...
    const jobs = await get(`${R}/actions/runs/${r.id}/jobs`);
    for (const j of jobs.json.jobs || []) {
      ...
      if (j.conclusion === 'failure') {
        const ann = await get(`${R}/check-runs/${j.id}/annotations`);
        for (const a of ann.json || []) console.log(`     [${a.annotation_level}] ${String(a.message).slice(0, 240)}`);
      }
    }
  }
  ...
  if (ref.status === 200) {
    const obj = ref.json.object || {};
    let sha = obj.sha;
    if (obj.type === 'tag') sha = ((await get(`${R}/git/tags/${obj.sha}`)).json.object || {}).sha;
    console.log(`[tag] v0.2.0 → 提交 ${String(sha).slice(0, 7)}`);
  } else {
    console.log(`[tag] v0.2.0 HTTP ${ref.status}`);
  }

  const rel = await get(`${R}/releases?per_page=10`);
  const list = Array.isArray(rel.json) ? rel.json : [];
