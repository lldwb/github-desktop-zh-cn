...
- **GUI 产物随 Release 分发**：CI 在四个 runner 上分别构建，与单文件产物一并作为 Release 附件（`…-win32-x64-setup.exe` / `…-win32-x64.zip` / `…-darwin-arm64.dmg` / `…-darwin-x64.zip` / `…-linux-x64.AppImage` / `…-linux-x64.deb` 等），附 `SHA256SUMS`。也可自行 `npm run dist` 构建（见 `README.md`「方式一」）。
