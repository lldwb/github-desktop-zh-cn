...
// 模块导出名（见 docs/dict-v2/design.md）。本探针把每处出现的前 80 / 后 16 字符打出来，
// 并对几类高危上下文给出警告；最终判断由人做，探针只负责把证据摆齐。
//
// 用法：
//   node build/tools/check-replaceable.cjs <文案> [选项]
...
function main() {
  ...
  if (args.help || !args.text) {
    console.log(`用法：node build/tools/check-replaceable.cjs <文案> [选项]
