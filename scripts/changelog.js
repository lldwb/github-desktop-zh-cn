...
// 用法：node scripts/changelog.js <版本号> [--file <CHANGELOG 路径>]
//
...
function printHelp() {
  console.log(`用法：node scripts/changelog.js <版本号> [选项]
...
示例：
  node scripts/changelog.js 0.1.0`);
}
...
function main() {
  ...
  if (!args.version) {
    console.error('错误：缺少版本号（用法：node scripts/changelog.js <版本号>，--help 查看帮助）');
    process.exit(1);
  }
