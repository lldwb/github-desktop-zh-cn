...
function printHelp() {
  console.log(`用法：node scripts/release-assets.js <list|fetch|latest> [选项]

从官方 Release 产物里按需取文件，只走 HTTP Range，不下载整包（产物单个 250~330 MB）。
fetch 产出的目录与真实安装目录同形，可直接喂给其它脚本：

  node scripts/release-assets.js fetch --version 3.6.6-beta2 --platform macos --out tmp/release/3.6.6-beta2/macos
  node scripts/scan.js --path tmp/release/3.6.6-beta2/macos --version 3.6.6-beta2
...
async function main() {
  ...
  try {
    ...
    console.log(`\n已提取到 ${r.appDir}`);
    console.log(`可用：node scripts/scan.js --path ${path.dirname(r.appDir)} --version ${args.version}`);
  } catch (e) {
    console.error(`错误：${e.message}`);
    process.exit(1);
  }
