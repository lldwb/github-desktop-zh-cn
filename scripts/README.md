...
- **打包态的环境差异（改脚本时注意）**：`process.execPath` 指向产物自身（不能当 node 用）；`tools/bundle.js` 复刻了 `require.main` 并指向入口 `cli.js`，因此子脚本的 `if (require.main === module) main()` 在打包态**不成立**——`cli.js` 透传子命令时显式调用脚本导出的 `main()`。打包器只收集静态 `require('...')` 字面量（模板字符串 / 变量拼接收集不到），JSON 模块会被转成 `module.exports = <JSON>`。**Electron 打包态（`npm run dist`）是另一套**：`__dirname` 落在 `resources/app.asar` 内（asar 内 `require` 正常），字典经 `extraFiles` 落到 exe 同级而非内嵌（`node:sea` 在 Electron 里不可用），上面关于 `require.main` 复刻的说明只适用于 bundle / SEA 产物。
