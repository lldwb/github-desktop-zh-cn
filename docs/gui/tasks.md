...
- [ ] **统一 v0.1.0 / v0.1.1 的附件命名**：先删掉这两个 tag 下各自遗留的**草稿**——页面上「发行版列表」里带「草案」标记的那些（2026-09-18 核实：v0.1.1、v0.1.0 各有一条，创建于当天 02:09 那批 `restore-release` 运行——同一批把正式 Release 发布了出去（`published_at` 02:10:30 / 02:10:41）却报了 failure（即 `fa96730` 修掉的「发布成功却报失败」），草稿就是那时留下的：建草稿 / 传附件 / 转正三步各自按 tag 找 Release，同 tag 下有两个候选时找到的可能不是同一个；已发布的正式 Release 不受影响），再跑 `repair-release.yml` 的 `rename-assets` 任务（tag 分别填 `v0.1.0`、`v0.1.1`）——这两个版本的附件缺 `cli` 通道词、macOS / Linux 还缺 `.bin` 后缀（规范见 AGENTS.md「产物命名」）。该任务只改名不动字节，`SHA256SUMS` 随之重算，旧名字的下载链接会失效；同 tag 还有草稿时它会直接停下并说明怎么清理（按 tag 寻址会有两个候选，见 AGENTS.md）
