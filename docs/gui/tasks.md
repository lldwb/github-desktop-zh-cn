...
- [ ] **统一 v0.1.0 / v0.1.1 的附件命名**：跑 `repair-release.yml` 的 `rename-assets` 任务（tag 分别填 `v0.1.0`、`v0.1.1`）——这两个版本的附件缺 `cli` 通道词、macOS / Linux 还缺 `.bin` 后缀（规范见 AGENTS.md「产物命名」）。该任务只改名不动字节，`SHA256SUMS` 随之重算，旧名字的下载链接会失效
