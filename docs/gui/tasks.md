...
- [x] **只复用自己的草稿**：署名在建 Release 那刻就定了（update 接口没有 author 字段，事后改不了），复用内置 token 建的草稿等于让最终 Release 署名 `github-actions[bot]`。续传前先比 `gh api user` 的 `.login` 与 `releases/tags/<tag>` 的 `.author.login`，对不上就 `gh release delete --yes` 后重建——远端那个 run #6 留下的 v0.2.0 草稿正是这种
