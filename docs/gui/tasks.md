...
- [x] 发布步骤改为**复用草稿续传**（run #10 的「草稿删掉重建」意味着已传附件全部作废重传，而 10 个附件约 1.4 GB 本就难一次传完）：**内容一致**的附件跳过（本地 `sha256sum` 比远端附件的 `.digest`——上传内容的 sha256；这正是本仓库要发版时那个草稿的用武之地：它带的是 fa8463f 那次的 5 个 SEA 产物，而 `scripts/common.js` 之后改过，digest 对不上就会重传，不会因尺寸恰好相同而留着旧产物）、缺什么补什么，发布前把不属于本次构建的遗留附件 `gh release delete-asset --yes` 清掉——末端状态与「删掉重建」等价。另把 `gh release edit` 补上 `--notes-file`（复用草稿时不走 create，正文得在这里写入）
- [x] 发布步骤控制流的离线验证：从 YAML 导出该步骤脚本（`tmp/step-test/release-step.sh`），用假 `gh`（记录 upload 内容的 sha256 当 digest）跑五种情形——① 无 Release → 建草稿、传 3 个、发布，退出码 0；② 草稿里的附件与本地内容一致 → `草稿里已有且内容一致，跳过：SHA256SUMS`、补传其余、发布，退出码 0；③ 草稿里同名同大小但内容是旧产物的 → 不跳过、`--clobber` 重传，退出码 0；④ 一个附件三次都传不上去 → 第 1/2 次失败各重试一次、第 3 次不再白等、`::error::` 点名该附件、**其余附件照传**、退出码 1、未发布（草稿留存供续传）；⑤ 已发布 → `Release v0.2.0 已发布，跳过`，退出码 0
