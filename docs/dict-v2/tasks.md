...
**修法**：脚本侧 `assertBaseUrl()` 在**发请求之前**校验地址（必须是带 `http(s)://` 前缀的绝对 URL），错了一次就报，并直说常见错法（漏前缀 / 被引号包住 / 混进空格或全角字符）；`collapseReasons()` 把失败条目的原因合并计数后写进报告 `reasonDetail`（同因合并、按条数排序、只留 3 类、压成一行），由 `::error::` 注解、step summary 与提交信息正文一并带出；进这些文本前先过 `redact()` 抹掉网关地址与密钥（`net.js` 的报错大多以「……：<URL 或主机名>」收尾，原样带出就等于把地址公开）。workflow 预检另加一道只判前缀的形态检查（两秒出结论）。**两处都只报形态、不回显值**——该值存 Secrets 的用意正在于此。
...
4. **单测**：新增 5 条（`assertBaseUrl` 放行/拦下且不回显值、`redact` 脱敏且不误伤短串、`collapseReasons` 合并计数与压行截断、`failureText`），`node --test` **89/89 全绿**。
