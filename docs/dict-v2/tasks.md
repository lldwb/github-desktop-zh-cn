...
- [x] `workflow_dispatch` 回填实测：指定一个此前无字典的官方版本，产出并校验通过
      ——**只能在真实 CI 上做**（本地无法触发 Actions）。脚本侧的多版本路径已就绪：`--version` 收逗号分隔列表并按版本升序依次产出，逐版本独立成败（一个版本失败不影响后续，job 最终状态为失败）；提交步骤用 `if: always()`，把成功的部分照样提交上去。**2026-09-19（UTC）在真实 CI 上跑通两版**（`AI_BASE_URL` 的值在环境侧改对之后，起因见 6.5）：3.6.0 产出 common 1817 / macos 238、新译 1 / 无需翻译 10 / 未译 0，干跑生效 98.7%，提交 `f6e269f` 已推送；3.6.4 产出 common 1837 / macos 231、新译 0 / 无需翻译 10 / 未译 0，干跑 98.6% / 98.8%，提交 `38b0ce4` 已推送。两版此前都没有字典，从取产物到推送全程无人工介入
...
### 6.5 CI 首跑失败：`AI_BASE_URL` 配错，且失败原因查不到

**起因**：`字典自动更新` 首次真正跑起 AI 链路（`workflow_dispatch`，产出 3.6.4）即失败，25 秒结束、`产出字典` 步骤标红，汇总只给一句「失败 3.6.4：未译 11/11 条（100%）超过上限 30%」。

**现象与取证**：日志里三条关键行——`接口 ***，模型 deepseek-v4-flash，思考强度 max，超时 900s`（`***` 是 Secret 脱敏）、`第 1 批（11 条）失败：Invalid URL——改为逐条重试`、`逐条重试 11 条：补回 0 条，判为无需翻译 0 条，仍失败 11 条`。`Invalid URL` 是 Node `new URL()` 抛的 `TypeError`，抛点在 `net.js` 的 `openStream` 首行——即 **`AI_BASE_URL` 的值不是合法 URL**，每个请求都在发出去之前就失败了。旁证：步骤耗时 10 秒，而 11 条逐条重试的 500 ms 间隔加起来就有 5.5 秒，真实模型调用单条约 10 秒——**请求根本没发出去**，排除超时与「模型不听话」。同时段那次 `3.6.0` 的跑同样失败，而定时任务那次是「已有字典跳过」，即**此前 AI 链路从未真正跑通过**。

**两个真缺口**（不是同一个）：① 配置错在环境侧（Secrets 里那个值），但脚本对它**零校验**——错了要等两批加逐条重试全部失败、再翻过「未译 100%」这层结论才看得出来；② **失败原因写不进任何人看得见的地方**：公开仓库的 Actions **注解与 step summary 人人可看，job 日志只有仓库管理员能取**（本次取证时匿名拉日志得 403），只把原因留在日志里等于没留。

**修法**：脚本侧 `assertBaseUrl()` 在**发请求之前**校验地址（必须是带 `http(s)://` 前缀的绝对 URL），错了一次就报，并直说常见错法（漏前缀 / 被引号包住 / 混进空格或全角字符）；连**看不见的字符**也一并识出——不换行空格、零宽空格、BOM、串内空格这几种 `new URL()` 照收不误，只是把请求打到 `/v1%C2%A0` 这种地址上**静默 404**，不报错、肉眼更看不出差别，比 `Invalid URL` 还难查，故报错里直接给出码点（字符本身看不见，说不出是哪一个就没法照着改）；反过来，URL 标准自己会删掉的制表符 / 换行与首尾半角空格不拦（拦了是误伤——粘贴时折行是常事）。`collapseReasons()` 把失败条目的原因合并计数后写进报告 `reasonDetail`（同因合并、按条数排序、只留 3 类、压成一行），由 `::error::` 注解、step summary 与提交信息正文一并带出；进这些文本前先过 `redact()` 抹掉网关地址、**主机名**与密钥——`net.js` 的报错大多以「……：<URL 或主机名>」收尾，而网络层那几条（`连接被拒绝（可能被防火墙拦截）：主机:端口`、请求超时）**只带主机名**，拿完整地址当 needle 匹配不上、会整条漏出去（实测确认），故 needle 里必须有 host。workflow 预检另加一道只判前缀的形态检查（两秒出结论）。**两处都只报形态、不回显值**——该值存 Secrets 的用意正在于此。

**实测取证**（2026-09-19，本机，复用已下载的 3.6.4 双平台产物，假服务器造错）：

1. **地址拼错**（`--ai-base gw.example.com/v1`）：第 4 步开跑前即失败，注解与报告都给出「AI_BASE_URL 不是合法 URL……」，**不再有几十条重复的 `Invalid URL`**。
2. **接口整轮失败**（假服务器 302 自跳转，复刻本次 CI 的失败形态）：门槛结论一字未变（仍是「未译 11/11 条（100%）超过上限 30%」），但注解、日志与报告都带上了 `重定向次数超过 5 次：***/chat/completions ×11`——**结论 + 原因**一起出现，地址已脱敏。8/8 项断言通过。
3. **workflow 两段内联 JS**（提交信息、汇总）抽出来用真实报告跑：失败行的明细、成功行的未译明细都按预期渲染，文案里不含地址；预检的前缀判断对大小写不敏感、能拦下「漏前缀」与「整段带引号」。13/13 项通过。
4. **单测**：新增 6 条（`assertBaseUrl` 放行/拦下且不回显值、**不可见字符拦下且报出码点**、`redact` 脱敏且不误伤短串、`collapseReasons` 合并计数与压行截断、`failureText`），`node --test` **90/90 全绿**。CI 首跑暴露的一处噪音已修：用例里那次「首尾空白」调用会把 `::warning::` 打进测试日志——默认 reporter 当注释原样带出，读日志的人会以为 Secret 配错了（换 reporter 时更会被 GitHub 认成一条真注解），现改为把 `console.warn` 截下来断言、日志里不再出现。
5. **网络层失败的主机名脱敏**（本轮补测）：地址指向必定连不上的端口（`--ai-base http://127.0.0.1:1/v1`），报错文本是 `连接被拒绝（可能被防火墙拦截）：127.0.0.1:1`——**只带主机名、不带完整地址**。补 needle 前实测：同一份文本原样漏出主机名；补上 host 后公开链路（注解 / 未译原因行 / 失败清单）里查不到它，明细显示为 `连接被拒绝（可能被防火墙拦截）：*** ×11`。6/6 项通过。
6. **地址形态清单**（同上）：12 种「复制粘贴污染」形态逐条过 `new URL()` 与 `assertBaseUrl()`——被拦下的是不换行空格 / 零宽空格 / 串内空格（码点各为 U+00A0 / U+200B / U+0020），放行的是尾随反斜杠、全角句点、缺端口、大写协议等**其实可用**的写法；制表符与串内换行经复测改为放行（URL 标准会把它们从整串里删掉，原先的判据会把「粘贴时折了行」误拦）。

**留给环境侧的动作**：`AI_BASE_URL` 这个 Secret 的值要改（本次未动它，也无权读它）——到仓库 Secrets 页按上面的常见错法核对一遍。第 6 组那条未勾选项（`workflow_dispatch` 回填实测）仍待配置修好后复跑。

**环境侧已改对，链路闭环**（2026-09-19 UTC）：值为 3.6.0 与 3.6.4 连跑两次 `workflow_dispatch` 回填，两版都产出成功并推送（`f6e269f`、`38b0ce4`），上一条待办随之关闭——**这条链路第一次真正在 CI 上跑通**。落地后复查失败可见性与脱敏：两次运行里网关地址在全部步骤日志中**一次都没出现**（`接口 ***`，脱敏生效），汇总为「成功 3.6.0：common 1817 / macos 238，新译 1，无需翻译 10，未译 0」与「成功 3.6.4：…未译 0」。产物写入后 `dict-edit` 的唯一提示是 `GitHub Copilot` 一条「译文与原文相同」——产品名保持英文，四个版本字典里都有这条，与本轮改动无关。
