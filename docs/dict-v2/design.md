...
1. **思考强度与超时要联动**。推理模型的耗时随思考强度非线性增长——实测同一个简单问题，接口默认档 10 秒 / 0.5k 思考 tokens，最高档 97 秒 / 12k（约 23 倍，参数名就是请求体的 `reasoning_effort`）。所以超时不能写死：`AI_TIMEOUT_SEC` / `--ai-timeout` 可配（默认 120 s），开了高思考强度却不放大超时的话每批都会超时、退化成逐条重试——更慢，请求数还翻倍。两者都用**仓库变量**而不是 Secrets（非敏感配置，可见比藏起来更好排查）。
